...
export const dump = (filePath: string, records: VueSFCAnalyzerRecords) => {
  ...
  logger.info(chalk.bold(`WebpackVueSFCAnalyzerPlugin saved stats file to ${filePath}`));
};
...
export const show = (records: VueSFCAnalyzerRecords) => {
  Object.keys(records).forEach((filePath) => {
    ...
    logger.info(chalk.underline(chalk.green(`[Compiled] ${filePath}:`)));
...
export const total = (records: VueSFCAnalyzerRecords) => {
  ...
  logger.info(chalk.underline("Total all of .vue file:"));
...
const presentEachSection = (analysis: { template: number, script: number, style: number }) => {
  ...
  logger.info(`  ${chalk.redBright("<template>")}: ${analysis.template} bytes (${Math.round(analysis.template / total * 100 * 10) / 10}%)`);
  logger.info(`  ${chalk.blueBright("<script>")}  : ${analysis.script} bytes (${Math.round(analysis.script / total * 100 * 10) / 10}%)`);
  logger.info(`  ${chalk.magentaBright("<style>")}   : ${analysis.style} bytes (${Math.round(analysis.style / total * 100 * 10) / 10}%)`);
};
